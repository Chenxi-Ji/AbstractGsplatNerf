...
def main(setup_dict):
    ...
    start_arr = np.array([-3.79, 0.69, -0.89])#np.array([0.63, -3.68, -2.34])*3
    end_arr = np.array([0.0, 10.0, 0.0])
    ...
    inputs_ref = generate_single(input_min, input_max, input_ref)#generate_samples(input_min, input_max, input_ref, N_sample=5, include_given=False)#generate_trajectory(input_min, input_max, input_ref, N_samples) # [N_samples+3, N]
