...
def main():
    ...
    if args.folder is None:
        folder =  "Outputs/AbstractImages/"+object_name+"/"+domain_type
    else:
        folder = args.folder
    if args.save_folder is None:
        save_folder = "Outputs/AbstractGif/"+object_name+"/"+domain_type
    else:
    ...
    indices = [int(f.split('_')[1].split('.')[0]) for f in images_dict if f.startswith("ref_")]
    ...
    frames[0].save(save_path, save_all=True, append_images=frames[1:], duration=600, loop=0)
