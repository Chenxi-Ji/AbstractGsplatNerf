...
def main(setup_dict):
    ...
    bg_colors = []
    for filename in os.listdir(bg_img_path):
        file_path = os.path.join(bg_img_path, filename)
        if os.path.isfile(file_path) and filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif')):
            bg_img = Image.open(file_path).convert("RGB")  # ensure RGB
            bg_img = bg_img.resize((height, width), Image.LANCZOS) 
            bg_img = np.array(bg_img, dtype=np.float32)
            bg_color = torch.from_numpy(bg_img/255).to(DEVICE)
    ...
    inputs_ref = generate_trajectory(input_min, input_max, input_ref, N_samples) #generate_single(input_min, input_max, input_ref)# [N_samples+3, N]
    inputs_ref = inputs_ref.to(device = DEVICE, dtype=DTYPE)
    #print("inputs_ref:",inputs_ref)
    ...
    pbar = tqdm(total=len(bg_queue),desc="Processing inputs", unit="item")

    while bg_queue:
        bg_color = bg_queue.pop(0)
        input_ref = inputs_queue[absimg_num%len(inputs_queue)]#random.choice(inputs_queue)#inputs_queue.pop(0) # [N, ]
...
if __name__=='__main__':
    ...
    bg_img_path = "./BgImg/Mountains"

    domain_type = "3"

    save_folder = "Outputs/RenderedImages/"+object_name+"/bg_2"
    save_ref = True
    save_bound = True
    ...
    #3
    input_min = torch.tensor([6, np.deg2rad(10), np.deg2rad(-15)]).to(DEVICE)
    input_max = torch.tensor([7, np.deg2rad(15), np.deg2rad(15)]).to(DEVICE)
    # yaw
